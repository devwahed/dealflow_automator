...
def process_uploaded_file(self, file_data_b64, filename, user_id):
    try:
        ...
        # ---------------- Keyword rules ----------------
        # Weak Tier 4 keywords
        TIER_4_KEYWORDS_WEAK = [
        ...
        def matches_tier_4_keyword(desc):
            if isinstance(desc, str):
                ...
                return any(keyword.lower() in desc_lower for keyword in TIER_4_KEYWORDS_WEAK)
            return False

        df["Keyword Match Weak"] = df["Description"].apply(matches_tier_4_keyword)
        df.loc[df["Keyword Match Weak"], "Pre-Product Tier"] = 4

        # Strong keywords
        TIER_4_KEYWORDS_STRONG = [
            "system integrator", "it consulting", "managed services",
            "grocery store", "retail", "farm supply", "agricultural co-op"
        ]
        TIER_1_KEYWORDS_STRONG = [
            "erp for", "hospital software", "construction software",
            "manufacturing automation", "fleet management software"
        ]

        def keyword_tier(description):
            if not isinstance(description, str):
                return None
            desc_lower = description.lower()
            if any(k in desc_lower for k in TIER_4_KEYWORDS_STRONG):
                return 4
            if any(k in desc_lower for k in TIER_1_KEYWORDS_STRONG):
                return 1
            return None

        df["Keyword_Tier_Strong"] = df["Description"].apply(keyword_tier)
        df.loc[df["Pre-Product Tier"].isna() & df["Keyword_Tier_Strong"].notna(),
        "Pre-Product Tier"] = df["Keyword_Tier_Strong"]

        # ---------------- GPT step ----------------
        df_gpt = df[df["Pre-Product Tier"] != 4].copy()  # Run GPT for all except obvious Tier 4
        ...
        def final_tier(pre_tier, gpt_tier):
            if pre_tier == 4:
                return 4
            if pre_tier in (1, 2):
                return 4 if gpt_tier == 4 else pre_tier
            return gpt_tier if gpt_tier else pre_tier

        df["Tier"] = df.apply(lambda r: final_tier(r["Pre-Product Tier"], r["Product Tier - CHAT GPT"]), axis=1)
        df["Post Tier"] = df["Tier"]  # Now consistent
        df["Post_Order"] = df["Post Tier"] * 10000 - df["Index"]

        # ---------------- Final filter & sort ----------------
        df = df[df["Tier"] != 4]  # Filter based on final tier
